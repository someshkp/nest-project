...
export class ProductsService {
  constructor(
    ...
    private readonly productReprository: Repository<ProductEntity>,
    private readonly categoryService: CatergoriesService,
  ...
  async create(
    ...
    const product = this.productReprository.create(createProductDto);
    product.category = category;
    product.addedBy = currentUser;

    return await this.productReprository.save(product);
  }
  ...
  async findOne(id: number) {
    const product = this.productReprository.findOne({
      where: { id: id },
      relations: {
        addedBy: true,
        category: true,
  ...
  async remove(id: number) {
    ...
    return await this.productReprository.remove(product);
  }
  ...
  async updateStock(id: number, stock: number, status: string) {
    ...
    product = await this.productReprository.save(product);
    return product;
  }
}
